...
const service = {
  ...
  nonAvailabilities: () => FIND_MANY_RESULT,
  nonAvailability: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("NonAvailability", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: NonAvailabilityService,
          useValue: service,
        },
      ],
  ...
  test("POST /nonAvailabilities", async () => {
    await request(app.getHttpServer())
      .post("/nonAvailabilities")
      .send(CREATE_INPUT)
  ...
  test("GET /nonAvailabilities", async () => {
    await request(app.getHttpServer())
      .get("/nonAvailabilities")
  ...
  test("GET /nonAvailabilities/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/nonAvailabilities"}/${nonExistingId}`)
  ...
  test("GET /nonAvailabilities/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/nonAvailabilities"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /nonAvailabilities existing resource", async () => {
